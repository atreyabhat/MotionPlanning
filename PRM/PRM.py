...
class PRM:
    ...
    def circle_sample(self, n_circles):
        """Use circle-based sampling with the start point as the center and store valid points
        arguments:
            n_circles - number of circles to sample

        Check collision for all points on each circle and append valid points to self.samples
        as [(row1, col1), (row2, col2), (row3, col3) ...]
        """
        # Clear previous samples
        self.samples = []
        ...
        for i in range(n):
            ...
            # Calculate the number of points to sample for current circle
            points_to_sample = int(2 * math.pi * radius*0.07)

            for j in range(points_to_sample):
                angle = 2 * math.pi * j / points_to_sample
                x = int(start[0] + radius * math.cos(angle))
                y = int(start[1] + radius * math.sin(angle))

                # if within the bounds of the map
                if 0 <= x < self.size_row and 0 <= y < self.size_col:
                    if self.map_array[x][y] == 1:  # if free space
    ...
    #     min_rx = 5  
    #     max_rx = 15
    #     #max number of circles depending on distance b/w start and goal
    #     n = int(dist*0.2)
    #     a=0
    #     b=0
    ...
    #         ry = rx   # eccentricity 1, circle
    #         #the sampling circles should be constrainted to not exceed a boundary
    #         if ry < dist*1.1:
    #             a+=rx
    #             b+=ry
    #         else:
    ...
    #         # Calculate the number of points to sample for current circle
    #         points_to_sample = int(2 * math.pi * a *0.05)

    #         for j in range(points_to_sample):
    #             angle = 2 * math.pi * j / points_to_sample
    #             x = int(start[0] + a * math.cos(angle))
    #             y = int(start[1] + b * math.sin(angle))
    ...
    def sample(self, n_pts, sampling_method="uniform"):
        ...
        if sampling_method == "uniform":
            ...
            max_weights = 50
            q_pairs = 130
        elif sampling_method == "circle":
            self.circle_sample(n_pts)
            max_weights = 30
            self.start_goal_radius = 30
            q_pairs = 50
